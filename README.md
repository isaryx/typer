...
**Bundled content:** Words from [first20hours/google-10000-english](https://github.com/first20hours/google-10000-english); passages from public-domain proverbs ([details](assets/PASSAGES_CREDITS.md)). For live quotes, ZenQuotes batch (`/api/quotes`) fills a local pool (one quote per session until empty, then refetch); optional `zenquotes-random` uses `/api/random`; [type.fit](https://type.fit/api/quotes) is the fallback. Toggle remotes with `typer set --quote-source` (see `typer credits` for attribution). Bundled `assets/quotes.json` is used when remotes are off or unavailable.
